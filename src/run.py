    ...
    for label, key, th_key, op in checks:
        if label in latest_values:
            ...
            if (op == '<' and val < thresholds[th_key]) or (op == '>' and val > thresholds[th_key]):
                signals += 1
    return int((signals / total) * 100), signals
...
def create_dashboard(data_results, yoy_results, thresholds):
    ...
    for i in range(16):
        ...
        if not is_yoy: # Level Chart
            ...
            ax.set_title(f"{label} (Level)", fontproperties=prop, fontsize=12)
            if "PMI" in label: ax.axhline(50, color='white', linestyle='--', linewidth=1)
        else: # YoY Chart
            ...
            ax.set_title(f"{label} (YoY %)", fontproperties=prop, fontsize=12)
