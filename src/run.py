...
def get_fred_data(indicators):
    ...
    for item in indicators:
        ...
        try:
            ...
            series = series.resample('MS').last().ffill() # 欠損値を補完
            ...
            print(f"⚠️ Warning: Could not fetch {label} ({series_id}): {e}")
            continue
    return data_results, yoy_results, latest_values

def calculate_recession_probability(latest_values, thresholds):
    signals, total_checks = 0, 6
    if latest_values.get('非農業部門雇用者数 (NFP)') and latest_values['非農業部門雇用者数 (NFP)']['yoy'] < thresholds['nfp_yoy_min']: signals += 1
    if latest_values.get('失業率') and latest_values['失業率']['yoy'] > thresholds['unrate_yoy_max']: signals += 1
    if latest_values.get('ミシガン大学消費者態度指数') and latest_values['ミシガン大学消費者態度指数']['value'] < thresholds['michigan_val_min']: signals += 1
    if latest_values.get('小売売上高') and latest_values['小売売上高']['yoy'] < thresholds['retail_yoy_min']: signals += 1
    if latest_values.get('米10年-2年金利差 (Yield Curve)') and latest_values['米10年-2年金利差 (Yield Curve)']['value'] < thresholds['yield_curve_max']: signals += 1
    if latest_values.get('ISM 製造業景況指数 (PMI)') and latest_values['ISM 製造業景況指数 (PMI)']['value'] < thresholds['pmi_min']: signals += 1
    return int((signals / total_checks) * 100), signals
...
def generate_report(latest_values, thresholds):
    ...
    lines = [f"# 【Weekly Macro Data】経済 Macro NOTE", f"📅 *最終更新: {today}*", "---", 
             f"## 📊 景気後退予測スコア: {prob}%", f"**判定結果: {status_msg}** ({signals_count} / 6 シグナル点灯)", "---"]
...
def create_dashboard(data_results, yoy_results, thresholds):
    ...
    for i, label in enumerate(labels):
        ...
        ax_l, ax_r = axes[row, col_base], axes[row, col_base + 1]
        data, yoy = data_results[label], yoy_results[label]

        # 実数値グラフ
        c_l = alert_color if (label == "ミシガン大学消費者態度指数" and data.iloc[-1] < thresholds['michigan_val_min']) or \
           (label == "米10年-2年金利差 (Yield Curve)" and data.iloc[-1] < thresholds['yield_curve_max']) or \
           (label == "ISM 製造業景況指数 (PMI)" and data.iloc[-1] < thresholds['pmi_min']) else normal_line
        ...
        if "PMI" in label: ax_l.axhline(50, color='white', linestyle='--', linewidth=1)

        # 前年比グラフ
        colors_r = [alert_color if (label == "非農業部門雇用者数 (NFP)" and v < thresholds['nfp_yoy_min']) or \
                    (label == "失業率" and v > thresholds['unrate_yoy_max']) or \
                    (label == "小売売上高" and v < thresholds['retail_yoy_min']) else normal_bar for v in yoy]
...
def main():
    try:
        ...
        create_dashboard(data, yoy, th); print("✅ Success! Dashboard updated with PMI.")
    except Exception as e: print(f"❌ Critical Error: {e}"); exit(1)
